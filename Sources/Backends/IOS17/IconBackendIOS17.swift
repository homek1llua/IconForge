...
final class IconBackendIOS17: IconBackend, @unchecked Sendable {
    ...
    let name = "iOS 17 Bundle Replace + Advanced Cache"
    ...
    func readIcon(for app: InstalledApp) async throws -> UIImage {
        ...
        let cachePath = findCachedIcon(for: app)
        if let cachePath, filesystem.fileExists(at: cachePath) {
            let data = try filesystem.readFile(at: cachePath)
    ...
    func applyIcon(_ image: UIImage, to app: InstalledApp) async throws {
        logger.info("iOS17 Backend: Applying icon to \(app.bundleIdentifier)")
        ...
        updateIconCache(for: app, data: iconData)
        clearSystemIconCaches(for: app)
        await refreshIcon(for: app)
        logger.info("iOS17 Backend: Successfully applied icon to \(app.bundleIdentifier)")
    }
    
    func restoreOriginalIcon(for app: InstalledApp) async throws {
        logger.info("iOS17 Backend: Restoring icon for \(app.bundleIdentifier)")
    ...
    private func updateIconCache(for app: InstalledApp, data: Data) {
        let cacheDirs = [
            paths.caches.appendingPathComponent("com.apple.IconsCache"),
            URL(fileURLWithPath: "/var/mobile/Library/Caches/com.apple.IconsCache"),
        ]
        for dir in cacheDirs {
            if filesystem.fileExists(at: dir) {
                ...
                try? filesystem.writeFile(data, to: cacheFile)
            }
        }
    }
    
    private func clearSystemIconCaches(for app: InstalledApp) {
        let patterns = [
            "/var/mobile/Library/Caches/com.apple.IconsCache/\(app.bundleIdentifier)*",
        ]
        for pattern in patterns {
            if let enumerator = FileManager.default.enumerator(atPath: (pattern as NSString).deletingLastPathComponent) {
                while let file = enumerator.nextObject() as? String {
                    if file.hasPrefix(app.bundleIdentifier) {
                        let fullPath = "\((pattern as NSString).deletingLastPathComponent)/\(file)"
